...
bag.survival=randomForest(x=mydataset[,32:520],y=mydataset$overall_survival,data=mydataset
                          ,subset=train,importance=TRUE, proximity = TRUE)
...
bag.survival2=randomForest(x=mydataset[,32:520],y=mydataset$overall_survival,data=mydataset
                          ,subset=train,importance=TRUE,ntree = 1000 ,proximity = TRUE)
...
for(i in 20:25) {
  temp.model <- randomForest(x=mydataset[,32:520],y=mydataset$overall_survival,data=mydataset
                             ,subset=train,importance=TRUE,ntree = 500 ,proximity = TRUE)
...
model <- randomForest(x=mydataset[,32:520],y=mydataset$overall_survival,data=mydataset
                      ,subset=train,importance=TRUE,ntree = 500 ,proximity = TRUE
...
conf_matr<-confusionMatrix(as.factor(round(boost_predict)), 
                as.factor(test$overall_survival))
conf_matr$table
conf_matr_ <-conf_matr$overall[1]
```

The performance when is predicted the test data set is lower than train data, it is because the algorithm overfit the train data and then don't do a great job with the test. Because is more sensible to the train data.
...
```{r}
boost_model_16=gbm(train$overall_survival~.,data= (train[, 32:520]), distribution="adaboost", n.trees=2000, interaction.depth=16, cv.folds = 3)
