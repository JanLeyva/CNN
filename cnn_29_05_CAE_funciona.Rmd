...
base_dir<-"~/KOLMOGOROV/Assigment_CNN/jan/CNN/malaria/malaria"
# train directories
...
```{r}
model %>% fit(train_generator, validation_generator, epochs = 10,
              callbacks = list(callback_model_checkpoint("cnn.h5", period = 2),
...
# Autoencoder
output_cifra<-predict(model10,validation_generator)
dim(output_cifra)


# From input to encoder
enc_output_cifra<-predict(model_enc10,validation_generator)
dim(enc_output_cifra)

# From encoder to decoder
dec_output_cifra<-predict(model_dec10,enc_output_cifra)
...
```{r plot output}
par(mfrow = c(2,2))
plot(as.raster(output_cifra[2,,,]))
plot(as.raster(enc_output_cifra[2,,,]))
plot(as.raster(dec_output_cifra[2,,,]))
```
```{r}
library(OpenImageR )
imageShow(dec_output_cifra[2,,,]/255)
imageShow(output_cifra[2,,,])
```
...
model_dec %>%
  layer_conv_2d(filters = 2, kernel_size = c(3,3), 
                ...
                input_shape =c(3,3,2) )  %>%
  layer_upsampling_2d(size = c(2,2))%>%
  layer_conv_2d(6,kernel_size = c(3,3), activation = "relu",padding = "same")%>%
  layer_upsampling_2d(size =  c(2,2))%>%
  layer_conv_2d(8, kernel_size =c(3,3), activation = "relu")%>%
  layer_upsampling_2d(size =  c(2,2))%>%
  layer_conv_2d(8, kernel_size =c(3,3), activation = "relu" )%>%
  layer_conv_2d(1, kernel_size =c(3,3), activation = "relu" )%>%
  layer_upsampling_2d(size =  c(4,4))
...
history <- model %>% fit(
  x=batch[[1]] , y =batch[[1]] ,   # Autoencoder
...
# Autoencoder
output_cifra<-predict(model,validation_generator)
dim(output_cifra)


# From input to encoder
enc_output_cifra<-predict(model_enc,validation_generator)
dim(enc_output_cifra)

# From encoder to decoder
dec_output_cifra<-predict(model_dec,validation_generator)
dim(dec_output_cifra)
```
...
idx<-1
#x_test_cifra[idx,,,1]
im<-matrix(validation_generator[idx,,,1], nrow=64, ncol=64) 
